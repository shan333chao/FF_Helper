...
void SignaturesDataSingleton::InitSignatureDataAddr()
{
	...
	mapSignatureDesc.insert(std::make_pair("PICK_MONSTER_CALL", "85 C0 75 EF 6A 01 56 8B CF E8 &?? ?? ?? ?? 8B C6 5F 5E 5B C2 08 00"));
	...
	mapSignatureDesc.insert(std::make_pair("SKILL_CALL", "E8 ?? ?? ?? ?? 50 8B CB E8 ?? ?? ?? ?? E9 ?? ?? ?? ?? FF 77 ?? 8B 4D ?? 50 E8 &?? ?? ?? ?? E9 ?? ?? ?? ?? 83 F8 0B 0F 85 ?? ?? ?? ?? A1 ?? ?? ?? ?? 8B 58 ?? "));
	...
	mapSignatureDesc.insert(std::make_pair("SKILL_REMOTE_PARAM1", "8B 0D &?? ?? ?? ?? 6A 00 53 56 E8 ?? ?? ?? ?? 85 C0 A1 ?? ?? ?? ??"));
	mapSignatureDesc.insert(std::make_pair("SKILL_REMOTE_CALL", "8B 0D ?? ?? ?? ?? 6A 00 53 56 E8 &?? ?? ?? ?? 85 C0 A1 ?? ?? ?? ??"));
	...
	mapSignatureDesc.insert(std::make_pair("PICK_FLY_CALL","8B 8B ?? ?? ?? ?? 3B CE 74 14 6A 01 57 E8 &?? ?? ?? ?? 8B 87 ?? ?? ?? ?? 89 83 ?? ?? ?? ?? 0F B6 05 ?? ?? ?? ?? A3 ?? ?? ?? ?? "));
	mapSignatureDesc.insert(std::make_pair("FLY_ATK_CALL","57 57 FF B3 ?? ?? ?? ?? B9 ?? ?? ?? ?? 6A 1D E8 &?? ?? ?? ?? E9 ?? ?? ?? ?? 8B 43 ?? 25 00 0C 00 00 BE 00 04 00 00 3B C6 0F 84 ?? ?? ?? ?? "));
